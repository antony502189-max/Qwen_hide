...
Current target-machine matrix: host `WDA_EXCLUDEFROMCAPTURE` is read back as `0x11`; legacy GDI screen copy is **Exposed** and therefore unsupported. Desktop Duplication produces a **RedactedPlaceholder** (not Qwen content, but not proven absent). This says nothing about Windows Graphics Capture or conferencing applications.
