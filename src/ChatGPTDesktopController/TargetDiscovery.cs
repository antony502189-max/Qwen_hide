...
public sealed record ChatGPTTarget(int ProcessId, IntPtr Hwnd, string ProcessName, string ExecutablePath, string WindowTitle, string WindowClass, long ProcessStartUtcTicks)
{
        ...
        foreach (var process in Process.GetProcessesByName("ChatGPT Classic"))
        {
            try
            {
                ...
                foreach (var hwnd in Native.TopLevelWindows((uint)process.Id))
                {
                    ...
                    candidates.Add(new(process.Id, hwnd, process.ProcessName, path!, Native.WindowText(hwnd), Native.WindowClass(hwnd), TryStart(process)));
                }
            }
